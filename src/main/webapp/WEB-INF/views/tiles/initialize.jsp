...
<div id="content">
	<div class="page-header">
		<h1 class="pretty">
			${actualSpecialization.name} <small>szakirány</small>
		</h1>
	</div>
	<c:forEach var="i" begin="1" end="${numberOfSemesters}" step="1">
		<div id="semester${i}" class="semester">
		    <span class="label label-info">${semesterWithRomanNums[i-1]}. félév</span>
			<c:forEach var="subject" items="${subjects }">
				<c:if test="${subject.offeredSemester eq i}">
					<button id="${subject.id}"
						class="dependencyBtn dependencyBtn-standby"

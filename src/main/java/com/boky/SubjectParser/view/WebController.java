...
public class WebController {
	...
	@RequestMapping(value = { "kotelezo", "geotechnikai", "magasepitesi",
			"telepulesi", "tuzvedelmi" }, method = RequestMethod.GET)
	public String getInitializationDatas(HttpServletRequest request, Model model) {
		String actualSpecialization = request.getRequestURI()
				.substring(request.getRequestURI().lastIndexOf("/") + 1)
				.toUpperCase(); // /SubjectParser/kotelezo -> KOTELEZO

		InitializationDataForWebDTO initializedData = facade
				.getInitializedData(actualSpecialization);

...
/**
 * This class needed because in the web service the autowired does not work. The
 * spring context is available via web service context but the update does not
 * work. The solution is the manual transaction handling so I organize this
 * logic to this class.
 * 
 * @author bokor
 * 
 */
@Service
public class WebServiceHelper extends SpringBeanAutowiringSupport {
	...
	public void updateSubject(SubjectForEditDTO paramSubject) throws WebServiceException {
		...
		if (subjectForUpdate == null) {
			throw new WebServiceException("There is no subject with this id");
		}
	...
	public void saveSubject(SubjectForEditDTO subject) throws WebServiceException {
		...
		if (subjectDao.getById(subject.getId()) != null) {
			throw new WebServiceException("A subject already exists with id: " + subject.getId());
		}
